...
public class RunningOrderListAdapter extends BaseAdapter {
	...
	public RunningOrderListAdapter(Context context, List<CustomerOrderItemDTO> data) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = layoutInflater.inflate(R.layout.running_order_list_item_layout, null);
		...
		viewHolder.getUndoBtn().setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new WSClientImpl(context).updateOrderItemStatus(Config.getToken(context), data.get(_position).getId(), 0, 1, new StatusListener() {
		...
		viewHolder.getFinishBtn().setOnClickListener(new OnClickListener() {
			int _position=position;
			@Override
			public void onClick(View v) {
				final LoadingDialog dialog = new LoadingDialog(context);
				// New items list management
				new WSClientImpl(context).updateOrderItemStatus(Config.getToken(context), data.get(_position).getId(), 2, 1, new StatusListener() {
